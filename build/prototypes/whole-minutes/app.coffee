...
Utils.interval 0.5, ->
	...
	barTimeOut++
	if timeOut == 5
		if getHoursOf(recording.player.duration) >= 1 then maxInput = 2 else maxInput = 6
		jumpToTime( patchRCNInput.text )
	timeOut++
...
timeJump = ( input ) ->
	...
	maxInput = 2
	if maxInput == 6 and patchRCNInput.text.length != maxInput
		...
		if getHoursOf(recording.player.duration) >= 1 
			...
			# If it's after first input
				...
				if patchRCNInput.text.length == 0 or patchRCNInput.text.length == 3
					...
					patchRCNInput.text += input
	else if maxInput == 2 and patchRCNInput.text.length != maxInput
		patchRCNInput.text += input
...
jumpToTime = ( timeString, maxInput ) ->
	...
	if maxInput == 4
		...
		if newTimes.length == 2
			irrationalTime = (60^2 * parseInt(newTimes[0])) + parseInt(newTimes[1]*60)

	else 
		irrationalTime = parseInt(timeString) * 60
